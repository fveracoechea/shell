- Ensure code quality by using `just format`, `just lint`, and `just typecheck`
- When running dev (`just dev`) or long running scripts use herdr panes
- Use QML types and properties with clear ownership boundaries
- Keep state normalization in pure JavaScript models where QML tests can exercise it
- Keep Surface lifetime in Surface Manager and platform state in Feature Services
- Specify JavaScript function input and output types with JSDoc `@param` and `@returns` tags in typechecked `.js` files only; QML functions declare input types with QML parameter annotations and never carry JSDoc

## Agent skills

### Issue tracker

Issues and specs live in GitHub Issues. See `docs/agents/issue-tracker.md`.

### Triage labels

Triage uses the standard five-role label vocabulary. See `docs/agents/triage-labels.md`.

### Domain docs

Domain documentation uses a single-context layout. See `docs/agents/domain.md`.
